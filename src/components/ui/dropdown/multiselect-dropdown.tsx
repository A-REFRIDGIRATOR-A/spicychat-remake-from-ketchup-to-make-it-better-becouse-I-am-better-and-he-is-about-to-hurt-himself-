...
export function MultiselectDropdown({
  ...
  const defaultLabels = items.map((item) => item.label);
  const [selectedOptions, setSelectedOptions] = useState<string[]>([]);
  const [deselectedOptions, setDeselectedOptions] =
  ...
  return (
    <Dropdown isOpen={open} onClick={handleClick}>
      {({ open }): JSX.Element => (
        <>
          <DropdownButton
            ref={buttonRef}
          ...
          <DropdownItems
            ref={ref}
            className={cn(
              `border flex-col items-center border-white/50
            bg-black min-w-[10rem] outline-glow rounded-lg`,
              className,
              !open && "hidden"
            )}
            closeOnItemClick={false}
          >
            {mergedOptions.map((label, index) => {
